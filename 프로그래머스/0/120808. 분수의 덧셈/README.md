...
### 학습 노트
<p>처음에 내가 오류 났던 이유 : 같은 i값으로 여러 번 나눌 수 있는 경우를 고려하지 못하고 for문으로 i값을 증가시키면서 분자, 분모를 약분시키는 형식으로 진행함.</p>
<p>또 while문 조건 자체에 break 조건을 넣어서 진행하면 훨씬 쉬운데 계속 if문으로 whlie문을 break 시키려 함.</p>
<p>최대공약수를 구하는 함수를 만들어서, 재귀함수형식으로 풀어나가도 됨!</p>
